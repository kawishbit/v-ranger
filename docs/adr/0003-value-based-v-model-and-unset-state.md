...
The predecessor's `v-model` carried a **1-based index**: the native input ran `min="1"` to `max="{stops.length}"`, so `3` meant `stops[2]`. Two problems followed.
...
- Consumers who genuinely want an index must derive it, and the migration path from the predecessor is a breaking change (documented in a migration table).
...
**No unset state; clamp `null` to the first stop.** Simpler by one rendering mode, and rejected precisely because it re-creates the predecessor's bug in a tidier form — it answers a question the user didn't answer.
